...
LastEditTime: 2021-10-19 02:47:05
LastEditors: Yang jin
Description: In User Settings Edit
FilePath: /BERT-pytorch-old/bert_pytorch/trainer/warmup.py
'''
...
class WarmUpLR(_LRScheduler):
    ...
    def get_metric_lr(self, metric,tolerate = 0.8):
        '''
        ...
        if metric < self.loss_buffer * tolerate:
            self.m = float(metric)
            self.p = 0
        else:
            self.p += 1
    ...
    def get_trap_lr(self,smallest_lr=1e-7,weight_decay = 0.001):
        '''
        @msg:
        @param:
    ...
    def get_trig_lr(self,extend_weight = 1,smallest_lr=1e-8,weight_decay = 0.005):
        '''
        @msg:
        @param:
            smallest_lr:动态学习率的最小lr
    ...
    def step(self, metric=None, epoch=None):
        ...
        with _enable_get_lr_call(self):
            if epoch is None:
                self.last_epoch += 1
                if metric is None and self.last_epoch < self.warmup_epochs:
                    values = self.get_lr()
                else:
                ...
                elif metric is not None or self.last_epoch < self.warmup_epochs:
                    values = self.get_lr()
                else:
                    values = self.get_metric_lr(metric)
