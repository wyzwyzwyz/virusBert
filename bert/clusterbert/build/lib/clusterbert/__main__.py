...
LastEditTime: 2021-10-29 09:06:41
'''
import argparse
...
from .trainer import BERTEmbedding
...
def save_dict(datadict,path):
    ...
    with open(path +time.strftime('%m_%d',time.localtime(time.time()))+".pk", 'wb') as f:
...
def run_encoder(args, output_path):
    ...
    frag_encoder = BERTEmbedding(
            bert_encoder, hidden=args.hidden, with_cuda=True)
    
    datadict= frag_encoder.get_encodes(test_data_loader,False,args.pooling_strategy)

    if args.embed_path:
        save_dict(datadict,args.embed_path)
